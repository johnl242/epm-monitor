...
import { supabase, signIn, signOut as supabaseSignOut, getCurrentUser } from './lib/supabase';
    ...
    <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
          ...
          <p className="text-center text-gray-500 text-sm mt-4">Test: admin@demo.com / admin123</p>
        </div>
      </div>
    </div>
  );
...
const App = () => {
  ...
  const login = async (email: string, password: string): Promise<boolean> => {
    ...
    try {
      ...
      // Check company status and license
      ...
      setUser({
        ...
        licenseTier: company?.license_tier ?? 'trial',
        licenseSeats: company?.license_seats ?? 0
      });
  ...
  return (
    <AuthContext.Provider value={{ user, loading, login, logout }}>
      <DataProvider companyId={user?.companyId || ''}>
        <Router>
          <Routes>
